...
Route::post("/manage", function (Request $request){
    ...
    if ($request->quizAction == 1){
        $quiz->delete();
        return redirect('/manage/'.$quiz->id.'/'.$quiz->code)->with("success_message", "Deleted quiz \"$quiz->title\".");
    }
...
Route::get("/take/{id}", function($id){
    ...
    $quiz_master_lookup = array(
        1 => "James or Andrew Dixon, MSc",
        2 => "Phil Hwang, MSc",
        3 => "George Kafalis, MSc",
